...
const buildPolicy = () => {
  var principalId = "78eeaf40-87dd-11e7-bca2-1d9debedd5b4"; //this should come from the database based on the credentials above
...
//   policy.allowMethod(
//      AuthPolicy.HttpVerb.GET,
//     "/v1/applications/*/available-countries"
//   );
//   policy.allowMethod(
//     AuthPolicy.HttpVerb.GET,
//     "/v1/applications/*/terms-of-service"
//   );
//   policy.allowMethod(
//     AuthPolicy.HttpVerb.GET,
//     "/v1/applications/*/privacy-policy"
//   );
//   policy.allowMethod(AuthPolicy.HttpVerb.GET, "/v1/applications/*/min-version");
...
const validateToken = (token)=>{
    ...
    const publicKeys = await getPublicKeys(process.env.COGNITO_PROVIDER_URL);
    const key = publicKeys[decoded.header.kid];
    const claim = jsonwebtoken.verify(token, key.pem);
    const currentSeconds = Math.floor( (new Date()).valueOf() / 1000);
    if (currentSeconds > claim.exp || currentSeconds < claim.auth_time) {
      throw new Error('claim is expired or invalid');
    }
    if (claim.iss !== process.env.COGNITO_PROVIDER_URL) {
...
exports.optional = async (event) => {
  ...
  if (!event.authorizationToken) return buildPolicy();

  //validate cognito token
  const valid = validateToken(event.authorizationToken);

  if (valid) return buildPolicy();
